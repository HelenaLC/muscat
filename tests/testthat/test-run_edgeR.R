...
res <- run_edgeR(data, pb, design, contrast, verbose = FALSE)
res_by_cluster <- res[[1]]$`B-A`

test_that("run_edgeR", {
    # check that nb. of DE genes is 10 in ea. cluster
    n_de <- sapply(res_by_cluster, function(x) sum(x$FDR < 1e-6))
    expect_true(all(n_de == 10))
    
    # check that DE genes are correct
    de_gs <- lapply(res_by_cluster, function(x) 
        x$gene[order(x$FDR)][seq_len(10)])
