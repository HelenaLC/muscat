...
For the @Kang2018-demuxlet dataset, for example, we might consider removing the *Dendritic cells* and *Megakaryocytes* clusters, as these containg less than 50 cells across all samples.
...
```{r dr-ids, echo = FALSE, results = "asis", fig.height = 6, fig.width = 14, fig.cap = "Dimension reduction plots. Cells are colored by cluster ID (A) and group ID (B), respectively."}
...
For our dataset, the t-SNE and UMAP colored by `cluster_id`s (Figure \@ref(fig:dr-ids)A) show that cell-populations are well-separated from one another. INF-$\beta$ stimulation manifests as a severe shift in the low-dimensional projection of cells when coloring by `group_id`s (Figure \@ref(fig:dr-ids)B), indicating widespread, genome-scale transcriptiontal changes. 
...
knitr::include_graphics(file.path(system.file(
  "extdata", package = "muscat"), "ds_approaches.png"))
```
...
If your not happy with how the plot looks, here's an example of how to modify the `ggplot`-object from above in various ways:
...
For the @Kang2018-demuxlet dataset, we want to carry out a single comparison of stimulated against control samples, thus placing `"ctrl"` on the right-hand side as the reference condition: 
...
Alternatively, we specify a design matrix capturing the experimental design using `model.matrix` (package `r Rpackage("stats")`), and a contrast matrix that specifies our comparison of interesting using `makeContrasts` from the `r Biocpkg("limma")` package. Alternatively, the comparison(s) of interest (or a list thereof) can be specified with via `coefs` (see `?glmQLFTest` for details).
...
## Cell-levels analysis: Mixed models {#sec-mmDS}
...
The code chunk generates a set of t-SNEs colored by gene expression for the top DS gene in each subpopulation. To facilitate matching the affected cells to their cluster and experimental group, we also add the t-SNEs colored by subpopulation and group ID, respectively, from above.  

```{r fig.width = 14, fig.height = 12, fig.cap = "t-SNE colored by gene expression. Show are t-SNE projections with cells colored by cluster and group ID (top row), and by the expression of the top DE gene for each cluster (bottom row)."}
...
Here, we generate violins plot for the top-12 DS genes (lowest adj. p-value) in each cluster. Note that, as we are testing for differential states at the cluster-level, we need to subset the cells that have been assigned to a given cluster for plotting. 

```{r eval = FALSE}
# generate violins for top hits by cluster
lapply(kids, function(k) {
  u <- sce[, sce$cluster_id == k]      # subset this cluster
  gs <- tbl_fil[[k]]$gene[seq_len(12)] # get top 12 hits
  
  plotExpression(u, features = gs, ncol = 4,
...
```{r violins, results = "asis", echo = FALSE, fig.width = 10, fig.height = 6, fig.cap = "Violin plots. Show are the top 12 hits (lowest adj. p-value) for a given cluster. Each violin is a sample; coloring corresponds to experimental groups (blue = control, orange = stimulated)."}
for (k in kids) {
  cat("### ", k, "{-}\n")
  print(plotExpression(
    sce[, sce$cluster_id == k], 
    features = tbl_fil[[k]]$gene[seq_len(12)],
    x = "sample_id", colour_by = "group_id", 
    point_size = 0.8, ncol = 4) +
...
Especially when wanting to gain an overview of numerous DE testing results for many clusters, bothm dimension reduction and cell-level visualisations require a lot of space can become cumbersome to interpret. In this setting, it is thus recommendable to visualise aggregated measures, e.g., mean expressions by cluster sample.
...
Alternatively, `pbHeatmap` provides a set of options regarding which cluster(s), gene(s), and comparison to include (arguments `k`, `g` and `c`, respectively).

For example, the following options render a heatmap visualizing the top 20 DS genes for the *B cells* cluster:

```{r pb-hm-2}
# top DS genes for single cluster
...
```{r pb-hm-3}
# single genes across all clusters
