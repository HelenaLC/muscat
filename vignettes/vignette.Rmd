...
```{r eval = FALSE}
# downsample to max. 200 cells per cluster
cs_by_k <- split(colnames(sce), sce$cluster_id)
cs200 <- unlist(sapply(cs_by_k, function(u) 
  sample(u, min(length(u), 200))))

# plot t-SNE & UMAP colored by cluster & group ID
for (dr in c("TSNE", "UMAP"))
  for (col in c("cluster_id", "group_id"))
    plotReducedDim(sce[, cs200], use_dimred = dr, colour_by = col)
```

```{r dr-ids, echo = FALSE, results = "asis", fig.height = 4, fig.width = 12, fig.cap = "Dimension reduction plots. Cells are colored by cluster ID (A) and group ID (B), respectively. For each cluster, at most 200 cells were sampled for plotting."}
...
for (dr in c("TSNE", "UMAP")) {
  ...
  ps <- lapply(c("cluster_id", "group_id"), 
    function(col) .plot_dr(sce[, cs200], dr, col = col))
...
```{r fig.width = 14, fig.height = 8, fig.cap = "t-SNE colored by gene expression. Show are t-SNE projections with cells colored by the expression of the top-8 DS genes. For each cluster, at most 200 cells were sampled for plotting."}
