...
The `r Biocpkg("scuttle")` and `r Biocpkg("scuttle")` packages provide a variety of tools for preprocessing and quality control of single-cell transcriptomic data. For completeness, we will apply some minimal filtering steps to
...
We use `computeRnaQcMetrics` to compute various per-cell quality control metrics, and proceed with filtering cells and genes as noted above:
...
The dimension reductions (DR) available within the SCE can be accessed via `reducedDims`. The data provided by @Kang2018-demuxlet already contains t-SNE coordinates; however, we can of course compute additional dimension reductions using one of `r Biocpkg("scrapper")`'s `runX` functions:

```{r umap}
# compute UMAP using 1st 20 PCs
sce <- runPca.se(sce, features=rownames(sce), number=20)
sce <- runUmap.se(sce)
```
