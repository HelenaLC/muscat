...
sim <- lapply(sim, runDR, dr = "TSNE", assay = "logcounts")
# arrange plots
ps <- lapply(c("cluster_id", "group_id"), function(id) 
    lapply(sim, function(u) plotDR(u, color_by = id)))
ps <- Reduce("c", ps)
...
sim <- lapply(c(0.01, 0.05, 0.1), function(u)
    simData(ref, p_type = u, nc = 1e3, nk = 3,
        ng = 1e3, force = TRUE))
# normalize & run dimension reduction
sim <- lapply(sim, logNormCounts)
sim <- lapply(sim, runDR, dr = "TSNE", assay = "logcounts")
...
ms <- c("edgeR", "DESeq2", "limma-trend", "limma-voom")
res <- lapply(ms, .run_method)
names(res) <- ms 
...
Last but not least, we may be interest in how methods perform across some grouping variable, for example, the DD categoy stored in `gene_info$category` of the simulated SCE's `metadata`. To this end, let's repeatt the above evalulation and visualization, but this time specify `category` as a facetting variable:

```{r tprfdr-facetted, warning = FALSE, message = FALSE, fig.wide = TRUE, fig.height = 6}
# calculate & visualize performance by DD category
perf <- .calc_perf(res, facet = "category")
pd <- prepare_data_for_plot(perf, incloverall = FALSE)
p <- plot_fdrtprcurve(pd) +
    theme(aspect.ratio = 1) +
    scale_x_continuous(trans = "sqrt")
